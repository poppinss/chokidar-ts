...
watcher.on('source:unlink', (filePath) => {
  // source file removed
})
...
The raw wrapper over chokidar is exported as follows:

```ts
import { Chokidar } from '@poppinss/chokidar-ts'
const chokidar = new Chokidar(__dirname)

chokidar.on('add', (filePath) => {
  // non ts file
})
chokidar.on('source:add', (filePath) => {
  // ts file
})
...
chokidar.on('source:change', (filePath) => {
  // ts file
})
...
chokidar.on('source:unlink', (filePath) => {
  // ts file
})
